 ...
 * 
 */
BasisFunction::BasisFunction(string AO_type, rowvec center, vec lmn, vec exponents, vec contraction_coeffs) {
...
vector<BasisFunction> Molecule::buildBasisFunctionsList() {
    ...
    for (int i = 0; i < nAtoms; i++) {
        ...
        if (atomicNumbers(i) == 1) {
            struct BasisFunction AO_1s = {"H-1s", coordinates.row(i), {0, 0, 0}, H_exponents, H_coefficients};
            basisFunctionsList.push_back(AO_1s);

        // If the atom is carbon, add 2s and 2p basis functions (4 in total)
        } else if (atomicNumbers(i) == 6) {
            struct BasisFunction AO_2s = {"C-2s", coordinates.row(i), {0, 0, 0}, C_exponents, C_2s_coefficients};
            struct BasisFunction AO_2px = {"C-2px", coordinates.row(i), {1, 0, 0}, C_exponents, C_2p_coefficients};
            struct BasisFunction AO_2py = {"C-2py", coordinates.row(i), {0, 1, 0}, C_exponents, C_2p_coefficients};
            struct BasisFunction AO_2pz = {"C-2pz", coordinates.row(i), {0, 0, 1}, C_exponents, C_2p_coefficients};
